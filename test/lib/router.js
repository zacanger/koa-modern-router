/* eslint-disable max-lines, max-lines-per-function, max-nested-callbacks, max-statements */
...
describe('Router', () => {
  ...
  it('does not register middleware more than once (gh-184)', (done) => {
    ...
    nestedRouter
      .get('/first-nested-route', (ctx) => { ctx.body = { n: ctx.n } })
      .get('/second-nested-route', (ctx, next) => next())
      .get('/third-nested-route', (ctx, next) => next())
  ...
  it('does not break when nested-routes use regexp paths', (done) => {
    ...
    nestedRouter
      .get(/^\/\w$/i, (ctx, next) => next())
      .get('/first-nested-route', (ctx, next) => next())
      .get('/second-nested-route', (ctx, next) => next())
  ...
  describe('Router#[verb]()', () => {
    ...
    it('registers route with a given name and regexp path', () => {
      const router = new Router()
      methods.forEach((method) => {
    ...
    it('registers array of paths with name', () => {
      const router = new Router()
      router.get('name', [ '/one', '/two' ], (ctx, next) => next())
      expect(router.stack).to.have.property('length', 2)
      expect(router.stack[0]).to.have.property('path', '/one')
      expect(router.stack[1]).to.have.property('path', '/two')
    })
  ...
  describe('Router#param()', () => {
    it('runs parameter middleware', (done) => {
      ...
      request(http.createServer(app.callback()))
        ...
        .end((err, res) => {
          ...
          res.body.should.have.property('name', 'zac')
          done()
        })
    })
    ...
    it('runs parameter middleware in order of URL appearance', (done) => {
      ...
      request(http.createServer(
        app.use(router.routes()).callback()))
        .get('/first/users/3')
        .expect(200)
        .end((err, res) => {
          if (err) return done(err)
  ...
  describe('router.routes()', () => {
    it('should return composed middleware', (done) => {
      ...
      request(http.createServer(
        app.use(routerMiddleware).callback()))
        .get('/users/1')
        .expect(200)
        .end((err, res) => {
          if (err) return done(err)
